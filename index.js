...
function elementId() {
    ...
    const elementId = `element-${randomNumber}`; 
    const element = document.getElementId(elementId); 
    if (element) {
        localStorage.setItem("elementId", elementId); 
        console.log(`Element ID: ${elementId} localStorage-e gonderildi`);
    } else {
        console.log(`Element ID: ${elementId} tapilmadi`);
    }
}
elementId();
...
function removeItem(key) {
    ...
    console.log(`${key} localStorage-den silindi.`);
}
...
function ages(users) {
    users.forEach(user => user.age = age(user.birthDate));
}
ages(users);
console.log(users);
